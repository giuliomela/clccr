...
#' }
simapro_export2 <- function(groups = F, critical = F, critical_type = "EU"){

  comp <- var1 <- comm <- um <- code1 <- code2 <- energy <- formula <- price <- NULL
  ...
  if (isTRUE(groups)) {
    ...
    data_raw <- simapro_codes |>
      ...
      dplyr::select(comp, var1, comm, price = mean, um, code1, code2, critical_eu, critical_iea)
