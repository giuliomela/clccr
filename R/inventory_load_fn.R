...
#' @return A tidy dataset with inventory data organized by project and object
#' @export
...
inventory_raw <- within(inventory_raw, {
  ...
  file_path <- NULL
  skip <- NULL
}) # defining a new variable containing the name of the object and of the project

# creating 2 new variables from the name variable: project and object names
inventory_raw <- tidyr::separate(inventory_raw,
                                 name, c("project", "object"), sep = "-")
...
inventory_raw_list <- dplyr::group_split(inventory_raw, project, object)

inventory_raw_list <- purrr::map(inventory_raw_list, function(x){
  tidyr::pivot_longer(x, !c(no:um, project:object),
