...
The `clccr_detail` function computes the relative share of each commodity considered for the computation of the CLCC indicator on the total indicator value, for both baseline and critical versions.
...
The user can specify the life cycle phase of interest with the `phase_of_int` parameter (i.e. `"total"`) and a parameter (`collapse_share`) that can be used to limit the number of commodities considered. For example, with `collapse_share = 0.9`, only commodities which shares sum up to 90% are considered and all the others are grouped in the `"others"` category.
