...
Below is an example of use of the `clcc` function. It is important to highlight that the name of each spreadsheet inventory file must follow a specific template, in order to be read correctly by the `clcc` function. This is designed to facilitate the work within the RSE company, in which the CLCC indicator is often calculated for several products (or objects) in many different projects (i.e.: smart mobility, stationary storage, etc.). The template to be followed to name correctly spreadsheet files is the following: `project_name-object_name.xlsx`. In the future, the CLCC function, can be modified to allow spreadsheets named differently as inputs.
...
Even though reference prices are loaded manually into the package folders, it is possible to retrieve the price matrix using the function `ref_prices`. The function does not need any argument.
